...
export const getGroupMembers = async (groupId) => {
  const query = `
    ...
    JOIN account a ON ag.account_id = a.account_id
    WHERE ag.group_id = $1;
  `;
...
export const requestToJoinGroup = async (accountId, groupId) => {
  const query = `
    INSERT INTO account_group_request (account_id, group_id)
    VALUES ($1, $2)
    RETURNING *;
  `;
  return await pool.query(query, [accountId, groupId]);
...
export const approveJoinRequest = async (requestId) => {
  ...
  await addMemberToGroup(account_id, group_id);
  await pool.query(`DELETE FROM account_group_request WHERE request_id = $1;`, [requestId]);
...
export const getJoinRequestsByGroup = async (groupId) => {
  const query = `
    ...
    JOIN account a ON agr.account_id = a.account_id
